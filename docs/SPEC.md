...
- 最近可比月的整体市场验收基准为 202602：MOM 销量约 `-14.8%`、MOM 销售额约 `-20.5%`、连续环比销量约 `+9.6%`、连续环比销售额约 `+23.8%`（2026-08-28 竞品快照更新后按新口径复算，替代原 64-94 父体口径下的 -27.7% / -0.1% / +9.3% / +33.5%；最终报告必须统一显示单位和精度）。
- 2026.01-06 已更新为全市场 listing 级导出（竞品快照批次 842085/842080/841734/841894/841906/841914，每月约 1038-1993 个父体，替代原 64-94 父体口径），2025 年仍为全市场行级口径（每月约 1683-2000 行）；两者量级一致、跨年同比可直接参考，但 2025 为行级导出（含变体行、无 ASIN 列）、2026 为父体级导出（父 ASIN 去重），颗粒度与导出日期仍略有差异，跨年结果必须随表注明，不得把剩余颗粒度差异误写成市场真实增长或下滑。2026.07 仍为 94 父体小口径，仅作附录/参考。
...
20. 基准值验收: 整体市场 202602 必须复现 MOM 销量约 -14.8%、MOM 销售额约 -20.5%、连续环比销量约 +9.6%、连续环比销售额约 +23.8%（2026-08-28 快照更新后新口径），显示单位和精度统一
...
### 7.1 新增数据源：竞品快照（2026.01-06 全市场 listing 级；2026.07 子体级）
...
- `dedup_YYYYMM`：按父 ASIN 去重（同一父 ASIN 多子体只保留 1 条；无父 ASIN 按自身保留）。**确定性规则：每个 Listing 键取源表第一条记录为代表行**（可复跑，替代旧随机抽样后固化的做法）。重放必须逐值复用该快照，不得重新随机抽样或擅自改选代表行，以确保 202602 等验收基准不漂移。
...
去重结果（2026-08-28 快照更新后）：202601=1134、202602=1038、202603=1766、202604=1744、202605=1690、202606=1993、202607=94 个父商品。
...
- 替换后 2026.01-06 为全市场父体级口径（1038-1993 商品/月），与 2025 全市场行级口径（1683-2000 行/月）量级一致，跨年对比可直接参考；2025 为行级导出（含变体行、无 ASIN 列）、2026 为父体级导出（父 ASIN 去重），剩余颗粒度差异报告须随表注明
...
| 同比范围标记 | 年度行输出 `scopeComparable/scopeNote`；2026.01-06 vs 2025.01-06 标记为可比（scopeComparable=true），但 scopeNote 必须注明剩余差异：2025 行级导出（含变体行、无 ASIN 列）vs 2026 父体级导出（父 ASIN 去重），颗粒度与导出日期仍略有差异 |
...
- 2026.01-06 为全市场父体级快照（1038-1993 商品/月，2026-08-28 更新），2025 年为全市场行级口径（1683-2000 行/月），量级一致、跨年同比可直接参考；剩余差异为 2025 含变体行/无 ASIN 列 vs 2026 父体去重，已随表注明
...
| data/raw/Competitor-US-2026.07-809440.xlsx | 1.29 MB | 竞品快照 2026.07（子体级 3000 行、94 父体，仅附录/参考） |
...
| src/build_competitor_db.js | 从 data/raw 竞品快照可复跑重建 competitor_809440.db（raw_ 镜像 + dedup_ 首条代表行，确定性规则） |
