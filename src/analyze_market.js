...
function addTrends(monthly) {
  ...
  for (let i = 0; i < monthly.length; i++) {
    ...
    // 用户口径 MoM = 跨年同月（今年X月 vs 去年X月，如 2025.01 vs 2024.01）
...
const data = {
  ...
  definitions: {
    ...
    momMonthly: '今年X月 vs 去年X月（跨年同月，用户口径 MoM）',
    chainMonthly: '本月 vs 上月（连续月环比）',
...
function mdMonthly(rows) {
  const out = ['| 月份 | SKU数 | 销量 | 销售额($) | SKU平均标价($) | 销量加权均价($) | MoM销量 | MoM销售额 | 环比销量 | 环比销售额 |',
    '|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|'];
  for (const r of rows) out.push(`| ${r.month} | ${fmt(r.skuCount)} | ${fmt(r.sales)} | ${fmt(r.revenue)} | ${fmt(r.avgListPrice, 2)} | ${fmt(r.weightedPrice, 2)} | ${fmtPct(r.momSales)} | ${fmtPct(r.momRevenue)} | ${fmtPct(r.chainSales)} | ${fmtPct(r.chainRevenue)} |`);
  return out.join('\n');
}
...
function mdSegments(rows) {
  const out = ['| 月份 | BSR分层 | SKU数 | 销量 | 销售额($) | MoM销量 | 环比销量 |',
    '|---|---|---:|---:|---:|---:|---:|'];
  for (const r of rows) out.push(`| ${r.month} | ${r.segment} | ${fmt(r.skuCount)} | ${fmt(r.sales)} | ${fmt(r.revenue)} | ${fmtPct(r.momSales)} | ${fmtPct(r.chainSales)} |`);
  return out.join('\n');
}

const labels = { overall: '整体市场', pp: 'PP塑料地垫', high: '非PP高客单产品', genimo: 'GENIMO品牌' };
const md = ['# 户外地垫市场分析报告（优化版）', '',
  `> 可比分析范围：${analysisMonths[0]}-${analysisMonths[analysisMonths.length - 1]}，共 ${analysisMonths.length} 个月（全部纳入，含 2026.03-07）。注意 2026.04 起源数据统计口径数量级突变，原值已如实披露，详见“异常附录”。`, '',
  '## 一、口径说明', '',
...
function monthlyHtml(rows) {
  return htmlTable(['月份', 'SKU数', '销量', '销售额($)', 'SKU平均标价', '销量加权均价', 'MoM销量', 'MoM销售额', '环比销量', '环比销售额'],
    rows.map((r) => [r.month, fmt(r.skuCount), fmt(r.sales), fmt(r.revenue), fmt(r.avgListPrice, 2), fmt(r.weightedPrice, 2), fmtPct(r.momSales), fmtPct(r.momRevenue), fmtPct(r.chainSales), fmtPct(r.chainRevenue)]));
}
...
function segmentHtml(rows) {
  return htmlTable(['月份', '小类BSR分层', 'SKU数', '销量', '销售额($)', 'MoM销量', '环比销量'],
    rows.map((r) => [r.month, r.segment, fmt(r.skuCount), fmt(r.sales), fmt(r.revenue), fmtPct(r.momSales), fmtPct(r.chainSales)]));
}
